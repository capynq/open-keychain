...
self.onmessage = async (event: MessageEvent<WorkerRequest>) => {
  ...
  try {
    ...
    if (request.type === 'generate') {
      ...
      self.postMessage(response, { transfer: [result.baseMesh.positions.buffer, result.baseMesh.indices.buffer, result.reliefMesh.positions.buffer, result.reliefMesh.indices.buffer] });
    } else {
      const { result, exportMesh } = await buildKeychain(wasm, request.params, true);
      if (!exportMesh || !result.printable) {
        const message = result.issues.find((issue) => issue.severity === 'error')?.message ?? 'This model is not ready to download.';
      ...
      const response: WorkerResponse = { type: 'export', requestId: request.requestId, filename, mimeType: format === '3mf' ? 'model/3mf' : 'model/stl', data };
