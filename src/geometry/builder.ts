...
function finalizeNameplate(
  ...
  deleteAll([carrierRawText, carrierCentered, carrierRotated]);
  ...
  const createStretchedRelief = (rootDepth: number): any => {
    ...
    deleteAll([raw, centered, stretched]);
    return placed;
  };
  for (let attempt = 0; attempt < 13; attempt += 1) {
    ...
    const candidateEmbedded = candidateParts.every((part: any) => {
      const overlap = plate.intersect(part);
      const embedded = overlap.numTri() > 0;
      overlap.delete();
      return embedded;
    });
    deleteAll(candidateParts);
...
export async function buildKeychain(
  ...
  if (params.templateId === 'nameplate') return finalizeNameplate(wasm, styled, params, issues, includeExport);
