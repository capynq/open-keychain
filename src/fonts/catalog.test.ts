...
describe('font script coverage metadata', () => {
  it('recognizes Cyrillic text and disables Latin-only families', () => {
    ...
    expect(latinOnly.map((font) => font.id)).toEqual(['quicksand', 'fredoka', 'bree-serif', 'baloo2', 'kalam', 'bungee']);
  ...
  it('includes the bilingual calligraphic families', () => {
    ...
    expect(calligraphic.map((font) => font.id)).toEqual(['caveat', 'marck-script', 'bad-script', 'neucha', 'amatic-sc', 'lobster', 'pangolin']);
    expect(calligraphic.every((font) => font.scripts.includes('latin') && font.scripts.includes('cyrillic'))).toBe(true);
