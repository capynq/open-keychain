...
describe('design document codec', () => {
  ...
  it('uses a compact, explicitly versioned payload', () => {
    const encoded = encodeDesignDocument({ version: 1, params: DEFAULT_PARAMS });
    expect(encoded.startsWith('v2.')).toBe(true);
    expect(encoded.length).toBeLessThan(
      btoa(JSON.stringify({ version: 1, params: DEFAULT_PARAMS })).length,
    );
  });
  ...
  it('decodes legacy v1 payloads', () => {
    const legacy = btoa(
      ...
      .replace(/\//g, '_')
      .replace(/=+$/g, '');
    expect(decodeDesignDocument(legacy)?.params.text).toBe('Legacy');
    expect(decodeDesignDocument(`v1.${legacy}`)?.params.text).toBe('Legacy');
  });
  ...
  it('rejects out-of-range and invalid colors', () => {
    expect(() =>
      encodeDesignDocument({ version: 1, params: { ...DEFAULT_PARAMS, textHeightMm: 31 } }),
    ).toThrow();
    expect(() =>
