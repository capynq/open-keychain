...
for (const font of FONT_CATALOG) for (const style of STYLE_CATALOG) for (const text of names) {
  if (!fontSupportsText(font, text)) continue;
  const { result, exportMesh } = await buildKeychain(wasm, { ...DEFAULT_PARAMS, fontId: font.id, styleId: style.id, text }, true);
...
console.log(JSON.stringify({ cases: passed + failed, passed, failed, maximumTriangles: Object.fromEntries(triangleCounts) }));
